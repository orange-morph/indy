...
var text1 : Text;
var text2 : Text;
var scrollSpeed : float = 6.0;

private var mapSections : int;
private var limit : int;
private var sectionWidth : int;
private var sectionNumber : int;
private var cameraStartPos : Vector3;
private var advancing = false;

function Start () {
	// The level was made with 16:9 screens in mind, and wider screens would ruin the seamless effect
	if (Camera.main.aspect > 16.0/9.0) {
		Camera.main.aspect = 16.0/9.0;
	}
	Tile.SetCamera();
	...
	var triggers : List.<Int2>;
	Tile.GetTriggerPositions (1, triggers);
	...
	var endPos = Int2(sectionWidth - 1, mapSize.y - 1);
	for (var i = 0; i < mapSections; i++) {
	...
	mapSize.x *= 2;
	Tile.NewLevel (mapSize, 0, 1.0, 0.0, LayerLock.None);
	Tile.SetAmbient (Color(.4, .4, .4, 1));
	
	// Paste each section into the new level twice, so scrolling can be seamless
	startPos.x = 0;
	for (i = 0; i < mapSections; i++) {
		Tile.SetMapBlock (startPos, mapDataSections[i]);
		Tile.SetMapBlock (startPos + Int2(sectionWidth, 0), mapDataSections[i]);
