...
public class SlotUI : MonoBehaviour {
	...
	{  
		...
		else if(InvContainer.DraggingItem == true)
		{
			if(InvContainer.ActiveContainer != null && ContainerItem == true && InvContainer.ActiveContainer.Slots[SlotID].IsTaken == false)
			{
				InvContainer.DragTarget = SlotID;
			}
			else if(InventoryItem == true && InvUI.InvManager.Slots[SlotID].IsTaken == false)
			{
				InvContainer.DragToInventory = true;
				InvContainer.DragTarget = SlotID;
			}
		}
		else if(EquipmentPanel.DraggingItem == true && InvUI.InvManager.Slots[SlotID].IsTaken == false)
		{
			if(InventoryItem == true)
			{
				EquipmentPanel.DragTarget = SlotID;
			}
		}
		else if(SkillBarPanel.DraggingItem == true && InvUI.InvManager.Slots[SlotID].IsTaken == false)
		{
			if(InventoryItem == true)
			{
				SkillBarPanel.DragTarget = SlotID;
			}
		}
	}
	...
	public void RemoveDragDestination () //When the drag item leaves a slot.
	{
		...
		if(InvContainer.DraggingItem == true) 
		{
			InvContainer.DragTarget = -1;
			InvContainer.DragToInventory = false;
		}
		if(EquipmentPanel.DraggingItem == true)
		{
			EquipmentPanel.DragTarget = -1;
		} 
		if(SkillBarPanel.DraggingItem == true)
		{
			SkillBarPanel.DragTarget = -1;
		} 
	}
